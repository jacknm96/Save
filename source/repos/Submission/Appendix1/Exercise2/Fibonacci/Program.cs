...
namespace Fibonacci
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            while (!int.TryParse(s, out a) || a <= 0) // loops until the user inputs a valid int (must be greater than 0)
            {
        ...
        static void WriteFibonacci(int n)
        {
            ...
            else
            {
                ...
                if (n > 2)
                {
                    for (int i = 2; i < n; i++) // calculates our digits of fibonacci and creates our output string simultaneously
                    {
